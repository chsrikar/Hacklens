...
1. **Push to GitHub**
   ...
   git remote add origin https://github.com/YOUR_USERNAME/hacklens.git
...
3. **Clone and setup**
   ```bash
   git clone https://github.com/YOUR_USERNAME/hacklens.git
   cd hacklens
   npm install
   cp .env.example .env
...
4. **Run with PM2 (process manager)**
   ```bash
   npm install -g pm2
   pm2 start server.js --name hacklens
   pm2 save
   pm2 startup
   ```
...
| Variable | Required | Description |
|----------|----------|-------------|
| `PORT` | No | Auto-set by platform (Render, Railway, etc.) |
| `NODE_ENV` | No | Set to `production` |
| `GITHUB_TOKEN` | No | GitHub PAT for higher rate limits |
| `OPENROUTER_API_KEY` | Yes | Required for AI summaries |
